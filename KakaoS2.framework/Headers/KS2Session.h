...
/*!
 ...
 @discussion    S2 이벤트 로깅 기능을 제공합니다.
                단일이벤트 전송방식과 일괄전송방식을 지원합니다.
 */
@interface KS2Session : NSObject

/*!
 @property  batchTime
 @abstract  이벤트 전송 주기. 단위 초.
 */
@property (assign, nonatomic) NSUInteger batchTime;
...
/*!
 @method    publishWithEvent:
 @abstract  단일 이벤트를 서버로 전송한다. defaultPublishCompletion이 호출 됨.
 @param event   전송할 KS2Event 객체.
 */
- (void)publishWithEvent:(KS2Event *)event;

/*!
 @method    publishWithAdidEvent:
 @abstract  인자로 넘어온 이벤트의 from값에 상관없이 Adid를 대입하여 서버로 전송한다. defaultPublishCompletion이 호출 됨.
 @param event   전송할 KS2Event 객체.
 */
- (void)publishWithAdidEvent:(KS2Event *)event;

/*!
 @method    publishWithEvent:completion:
 @abstract  단일 이벤트를 서버로 전송한다.
 @param event       전송할 KS2Event 객체.
 @param completion  전송 완료 콜백.
 */
- (void)publishWithEvent:(KS2Event *)event completion:(KS2PublishCompletion)completion;

/*!
 @method    publishWithAdidEvent:completion:
 @abstract  인자로 넘어온 이벤트의 from값에 상관없이 Adid를 대입하여 서버로 전송한다.
 @param event       전송할 KS2Event 객체.
 @param completion  전송 완료 콜백.
 */
- (void)publishWithAdidEvent:(KS2Event *)event completion:(KS2PublishCompletion)completion;
